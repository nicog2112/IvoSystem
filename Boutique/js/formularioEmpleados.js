...
const expresiones = {
	...
	cargo: /^[a-zA-ZÀ-ÿ\s]{3,40}$/, // Letras y espacios, pueden llevar acentos.
	legajo: /^\d{8,10}$/, // 8 a 10 numeros.
...
const validarFormulario = (e) => {
	switch (e.target.name) {
		...
		break;
		case "legajo":
			validarCampo(expresiones.legajo, e.target, 'legajo');
		break;
...
formulario.addEventListener('submit', (e) => {
	...
	if(campos.nombre  && campos.apellido && campos.dni && campos.nacionalidad && campos.legajo && 
		campos.fechaNacimiento && campos.sexo && campos.cargo){
