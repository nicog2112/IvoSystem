...
$lista = Perfil::obtenerTodos($filtroEstado);
...
if (isset($_GET["error"])) {
    ...
    case 'nombrePerfilAñadir':
        $mensaje = "<div class='error'>"."El nombre del Perfil solo puede contener letras, se debe escribir minimo 3 caracteres, los espacios no estan permitidos y no puede estar vacio."."</div>";
        break;

  

    case 'nombrePerfilModificar':
        $mensaje1 = "<div class='error'>"."El nombre del Perfil solo puede contener letras, se debe escribir minimo 3 caracteres, los espacios no estan permitidos y no puede estar vacio."."</div>";
        break;
...
<html lang="en">
    <head>
        ...
        <script src="/programacion_3/boutique/datatables/Buttons-1.5.6/js/buttons.html5.min.js"></script>
       	...
       	<link href="../../css/simple-switch.min.css" type="text/css" rel="stylesheet" />
        <!--datables CSS básico-->
        ...
        <main>
            <section id="contenedorPrincipal">
                        ...
                        <tbody>
                            <?php foreach  ($lista as $Perfil): ?>
                                <tr class="active-row">
                                    ...
                                    <?php if($Perfil->getIdPerfil() == 3){ ?>
                                    <td><button class="btnActualizar" id="abrir4" title="Bloqueado por seguridad"><i class="botonGeneral fas fa-lock"></i></button></td>
                                    <?php }; ?>
                                     <?php if($Perfil->getIdPerfil() != 3){ ?>
                                     <td><button class="btnActualizar" type="button" id="abrir5" value="<?php echo $Perfil->getIdPerfil(); ?>"  onclick="abrirModal(this.id,this.value);"><i class="botonActualizar fa fa-bezier-curve"></i></button></td>
                                     <?php }; ?>
                                     <td><a href="/programacion_3/boutique/modulos/usuarios/listado.php?id_perfil=<?php echo $Perfil->getIdPerfil(); ?>"><i class="botonGeneral fas fa-user"></i></a></td>
                                    
                                    <td><button class="btnActualizar" type="button" id="abrir2" value="<?php echo $Perfil->getIdPerfil(); ?>"  onclick="abrirModal(this.id,this.value);"><i class="botonActualizar fas fa-edit"></i></button></td>

                                    <?php if($Perfil->getIdPerfil() < 4){ ?>
                                    <td><button class="btnActualizar" id="abrir4" title="Bloqueado por seguridad"><i class="botonGeneral fas fa-lock"></i></button></td>
                                    <?php } ?>
                                     <?php if($Perfil->getIdPerfil() > 3){ ?>
                                        <td><button class="btnActualizar" id="abrir4" onclick="abrirModal(this.id,this.value);" value="<?php echo $Perfil->getIdPerfil(); ?>"><i class="botonEliminar fas fa-trash"></i></button></td>
                ...
                <div id="miModal4" class="modal">
                 ...
                 <!-- Ventana modal listado Modulos -->

                <div id="miModal5" class="modal">
                    <div class="flex" id="flex5">
                        <div class="contenido-modal">
                            <div class="modal-header flex">
                                <h2>Listado modulos del Perfil</h2>
...
<?php foreach ($listadoModulos as $modulo5): ?>
  ...
  ?>
    ...
    <script src="../../js/jquery.simpleswitch.min.js"></script>
