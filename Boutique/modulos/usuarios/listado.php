...
$idPerfilUsuario = $_GET["id_perfil"];


$lista = Usuario::obtenerTodos($idPerfilUsuario,$filtroEstado);
...
if (isset($_GET["error"])) {
    ...
    case 'usuarioExistente':
        $mensaje = "<div class='error'>"."El usuario que eligio o el dni ya existe en este perfil"."</div>";
        break;
    case 'username':
        $mensaje = "<div class='error'>"."El usuario tiene que ser de 4 a 10 caracteres"."</div>";
        break;
    case 'password':
        $mensaje = "<div class='error'>"."La contraseña tiene que ser de 4 a 12 caracteres"."</div>";
        break;
...
<html lang="en">
    <head>
        ...
        <script src="../../js/formularioModificarUsuario.js"></script>
        ...
        <main>
            <section id="contenedorPrincipal">
                ...
                <div id="containerNuevo">
                    <button class="botonAñadir" id="abrir" onclick="abrirModal(this.id,this.value);" value="<?php echo $idPerfilUsuario; ?>" ><i class="fas fa-plus"></i> Añadir </button>
                </div>
                <br>
                <form method="GET" class="formularioEstado">
                    ...
                    <input type="hidden" name="id_perfil" value="<?php echo $idPerfilUsuario; ?>">
                    <button type="submit" id="botonFiltrar">Filtrar</button> 
                </form>


                <div id="container">
                    <table id="tabla7" class="styled-table" >
                        <thead>
                            <tr>
                             	<th>ID</th>
                                <th>Imagen</th>
                        ...
                        <tbody>
                            <?php foreach  ($lista as $usuarioPerfil): ?>
                                <tr class="active-row">

                                   	<td><?php echo $usuarioPerfil->getIdUsuario(); ?></td>
                                    <td> <img src='<?php echo $usuarioPerfil->getImagen(); ?>'style="max-width:100px;width:100px;height:100px;"></td>
									<td><?php echo $usuarioPerfil->getUsername(); ?></td>
									<td><?php echo $usuarioPerfil->getNombre(); ?></td>
									<td><?php echo $usuarioPerfil->getApellido(); ?></td>
								
									<td><?php echo $usuarioPerfil->getDni(); ?></td>
                                    <?php
                                    $estado = $usuarioPerfil->getEstadoUsuario();
                                    $clase="";
                                    if ($estado == 1){
                                        $estado = "Activo";
                                        $clase= "estadoActivo";
                                    ...
                                    <td><button class="btnActualizar" type="button" id="abrir2" value="<?php echo $usuarioPerfil->getIdUsuario(); ?>"  onclick="abrirModal(this.id,this.value);"><i class="botonActualizar fas fa-edit"></i></button></td>

                                     <?php if($usuarioPerfil->getIdUsuario() == 1){ ?>
                                    <td><button class="btnActualizar" id="abrir4" value="<?php echo $usuarioPerfil->getIdUsuario(); ?> " title="Bloqueado por seguridad"><i class="botonGeneral fas fa-lock"></i></button></td>

                                     <?php } ?>
                                     <?php if($usuarioPerfil->getIdUsuario() != 1){ ?>

                                    <td><button class="btnActualizar" id="abrir4" onclick="abrirModal(this.id,this.value);" value="<?php echo $usuarioPerfil->getIdUsuario(); ?>"><i class="botonEliminar fas fa-trash"></i></button></td>
                                    <?php } ?>
                                </tr>
...
<input type="hidden" id="idPerfilUsuarioModificar" value="<?php echo $idPerfilUsuario; ?>">
